...
# Simple transform: read CSV, drop rows with nulls, write CSV to cleaned/<date>/data.<timestamp>.csv
try:
    ...
    # write a single CSV part to a temp folder
    df_clean.coalesce(1).write.mode('overwrite').option('header', 'true').csv(temp_folder)

    # find the part file and copy it to the final key data.<ts>.csv
